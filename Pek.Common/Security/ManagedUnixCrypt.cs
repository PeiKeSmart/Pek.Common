...
public static class ManagedUnixCrypt
{
    ...
    public static bool Verify(string hash, string password)
    {
        ...
        var newHash = Crypt(password, splittedHash.GetFullSalt());
    ...
    public static string GenerateSalt(string algoType = DefaultType, int rounds = RoundsDefault)
    {
        ...
        var saltByteCount = algoType == TypeMD5 ? 6 : 12;
        var randomBytes = new byte[saltByteCount];
    ...
    private static string CryptMD5(ArrayPointer<byte> key, ArrayPointer<byte> salt)
    {
        ...
        // MD5CryptR.
        var buflen = Strlen(md5SaltPrefix) +
          Strlen(salt) + 1 + 26 + 1;
    ...
    private static string CryptSHA256(ArrayPointer<byte> key, ArrayPointer<byte> salt)
    {
        ...
        var buflen = Strlen(sha256SaltPrefix) +
    ...
    private static string CryptSHA512(ArrayPointer<byte> key, ArrayPointer<byte> salt)
    {
        ...
        var buflen = Strlen(sha512SaltPrefix) +
    ...
    private static ArrayPointer<byte> MD5CryptR(ArrayPointer<byte> key, ArrayPointer<byte> salt, ArrayPointer<byte> buffer, int buflen)
    {
        ...
        var saltLen = Math.Min(Strcspn(salt, dollarSign), 8);
        var keyLen = Strlen(key);

        var temp = new byte[key.SourceArray.Length];
    ...
    private static ArrayPointer<byte> SHA256CryptR(ArrayPointer<byte> key, ArrayPointer<byte> salt, ArrayPointer<byte> buffer, int buflen)
    {
        ...
        var rounds = RoundsDefault;
        var roundsCustom = false;
        ...
        {
            var num = salt + Strlen(sha256RoundsPrefix);
            var srounds = Strtoul(num, out var endp);
            if (endp.Value == (byte)'$')
            {
                salt = endp + 1;
        ...
        var saltLen = Math.Min(Strcspn(salt, dollarSign), SaltLenMax);
        var keyLen = Strlen(key);
        ...
        if (roundsCustom)
        {
            ...
            var temp1 = (rounds.ToString() + "$\0").ToCharArray();
            var temp2 = new byte[temp1.Length];
            for (var i = 0; i < temp1.Length; i++) temp2[i] = (byte)temp1[i];
    ...
    private static ArrayPointer<byte> SHA512CryptR(ArrayPointer<byte> key, ArrayPointer<byte> salt, ArrayPointer<byte> buffer, int buflen)
    {
        ...
        {
            var num = salt + Strlen(sha512RoundsPrefix);
            var srounds = Strtoul(num, out var endp);
            if (endp.Value == (byte)'$')
            {
                salt = endp + 1;
    ...
    {
        var temp = hashAlg.ComputeHash(ctx.ToArray());
        for (var i = 0; i < temp.Length; i++, buffer++)
        {
            buffer.Value = temp[i];
        }
    }
    ...
    {
        for (var i = 0; i < length; i++, str1++, str2++)
        {
    ...
    private class SplittedHash
    {
        ...
        public static SplittedHash Parse(string str)
        {
            ...
            var ret = str.Split(new[] { '$' }, 4, StringSplitOptions.RemoveEmptyEntries);
            if (ret.Length < 3)
