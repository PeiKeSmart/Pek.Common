...
public static class Check
{
    ...
    /// <exception cref="ArgumentException"></exception>
    /// <exception cref="ArgumentNullException"></exception>
    public static void NotEmpty(Guid value, string paramName) => Require<ArgumentException>(value != Guid.Empty, string.Format("参数“{0}”的值不能为Guid.Empty", paramName));
    ...
    public static void NotNullOrEmpty<T>(IDictionary<string, T> dictionary, string paramName)
    {
        ...
        Require<ArgumentException>(dictionary.Any(), string.Format("参数“{0}”不能为空引用或空集合。"));
    }
