...
public static partial class DataExtension
{
    private sealed class DbParameterReadOnlyCollection(DbParameterCollection parameterCollection) : IReadOnlyCollection<DbParameter>
    {
        ...
        public Int32 Count => _paramCollection.Count;

        public IEnumerator<DbParameter> GetEnumerator() => (IEnumerator<DbParameter>)_paramCollection.GetEnumerator();
    ...
    /// <returns>A T.</returns>
    public static async Task<T?> ExecuteScalarToAsync<T>(this DbCommand @this, CancellationToken cancellationToken = default) => (await @this.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false))!.To<T>();
    ...
    /// <param name="cancellationToken"></param>
    /// <returns>A T.</returns>
    public static async Task<T?> ExecuteScalarToOrDefaultAsync<T>([NotNull] this DbCommand @this, CancellationToken cancellationToken = default) => (await @this.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false))!.ToOrDefault<T>();
