...
```csharp
public class DebugConfig : Config<DebugConfig>
{
    ...
    static DebugConfig()
    {
        try
        {
            RegisterConfigForAot(DebugConfigJsonContext.Default);
            Console.WriteLine("配置注册成功");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"配置注册失败: {ex.Message}");
        }
    }
    
    protected override void OnConfigLoaded()
    {
        Console.WriteLine($"配置已加载: {TestProperty}");
    }
    
    protected override void OnConfigSaved()
    {
