...
namespace MyApp
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            {
                // 初始化需要的配置（AOT 环境下）
                InitializeRequiredConfigs();

                Console.WriteLine("所需配置已成功初始化");
        ...
        // AOT 环境下初始化所需配置
        private static void InitializeRequiredConfigs()
        {
            // 方式1：使用ConfigInitializer初始化特定配置类
            ConfigInitializer.InitializeConfig<AppConfig>();
            ConfigInitializer.InitializeConfig<DatabaseConfig>();
            ConfigInitializer.InitializeConfig<LoggingConfig>();
            
            // 方式2：或者直接访问配置的Current属性触发初始化
            // var appConfig = AppConfig.Current;
            // var dbConfig = DatabaseConfig.Current;
            // var logConfig = LoggingConfig.Current;
